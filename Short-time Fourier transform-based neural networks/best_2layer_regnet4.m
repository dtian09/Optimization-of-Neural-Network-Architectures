function [bestnet,bestnettr,bestnet_average_rmse,bestnet_rmses,topologies_average_rmses] = best_2layer_regnet4(x,t,mini_nodes,max_nodes,m,trainFcn)
%Train 2-hidden layer mlps with mini_nodes to max_nodes in each layer
...
for h1=mini_nodes:max_nodes
    for h2=mini_nodes:max_nodes
        for i=1:m%train, validate and test each network topology m times
         %   if h1+h2 < 24 % train networks with < 24 hidden nodes
         %   if h1+h2 > 23 % train networks with at least 24 hidden nodes
                ...
                topologies_average_rmses=[topologies_average_rmses;[h1 h2 average_rmse]];
        ...
        %if h1+h2 > 23 %networks with at lease 24 hidden nodes are trained
        %if h1+h2 < 24
